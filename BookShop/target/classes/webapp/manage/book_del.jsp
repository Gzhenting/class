...
if (PID!=null && PID!=""){
    ...
    ResultSet rs = conn.executeQuery("select t1.ID, t1.typeID, t1.BookName, t1.introduce, t1.price, " +
            "t1.nowPrice, t1.picture, t1.newBook, t1.sale, t2.TypeName, t2.superType from tb_book as t1, " +
            "tb_subtype as t2 " +
	...
	if(!rs.next()){
	    ...
	    //赋值图书信息
        superID = rs.getString("superType");
        subType = rs.getString("TypeName");
    ...
    rs = conn.executeQuery("select TypeName from tb_supertype where ID=" + superID);
    if(!rs.next()) {
        out.println("<script lanuage='javascript'>alert('您的操作有误1!');window.location.href='index.jsp';</script>");
    }else{
        superType = rs.getString("TypeName");
    }
%>
<html>
<head>
	...
	<!---->
			    ...
			    <table width="94%"  border="0" align="right" cellpadding="-2" cellspacing="-2" bordercolordark="#FFFFFF">
                  ...
                  <tr>
                    ...
                    <td height="27">&nbsp;<%=price%>(元)</td><td height="27" align="left">现&nbsp;&nbsp;&nbsp;&nbsp;价： </td>
