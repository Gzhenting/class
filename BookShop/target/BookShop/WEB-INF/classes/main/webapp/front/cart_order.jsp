...
<%
	if (session.getAttribute("cart") == "") {//判断购物车对象是否为空
		out.println(
				"<script language='javascript'>alert('您还没有购物!');window.location.href='index.jsp';</script>");
	}
	...
	if (Username != "") {
		try {//捕捉异常
			...
			if (!rs_user.next()) {//如果获取的账户名称在会员信息表中不存在（表示非法会员）
				...
				String receiveName = chStr.chStr(request.getParameter("receiveName"));//获取输入的收货人姓名
				...
				String sql = "insert into tb_order(bnumber,username,receiveName,address,tel,bz) values("
						+ bnumber + ",'" + Username + "','" + receiveName + "','" + address + "','" + tel
						+ "','" + bz + "')";
				...
				for (int i = 0; i < cart.size(); i++) {
					Bookelement mybookelement = (Bookelement) cart.elementAt(i);//获取购物车中的一个图书
					ID = mybookelement.ID;//获取图书ID
				...
				if (!flag) {//如果订单无效
					...
					out.println("<script language='javascript'>alert('订单生成，请记住您的订单号[" + orderID
