...
<%
	...
	ResultSet rs = conn.executeQuery("select * from tb_book t1,tb_subType t2 where t1.typeID=t2.ID and t2.superType="
									...
									<div class="box-content">
										...
										<div class="row">
											<%
												...
												if (str == null) {//如果没有请求就设置为0
													str = "0";
												}
												int pagesize = 12;//设置页面显示数量的大小为12
												rs.last();//跳转到最后一条数据的信息
												int RecordCount = rs.getRow();//获取最后一条信息的行数
												int maxPage = 0;//定义最大页面数量
												maxPage = (RecordCount % pagesize == 0) ? (RecordCount / pagesize) : (RecordCount / pagesize + 1);//计算最大页面数

												int Page = Integer.parseInt(str);//将Page参数赋值给Page
												if (Page < 1) {//如果Page小于1从第一个开始
													Page = 1;
												} else {
													if (Page > maxPage) {//如果Page大于最大的页面数，赋值为最后一个页面
														Page = maxPage;
													}
												}
												rs.absolute((Page - 1) * pagesize + 1);//分页定位到查询页面第一条记录
